...
class DSRPTV_Geolocation{
	...
	*/
	...
	public static function get_data( $from_cookie = true ){
		...
		if( $from_cookie && isset( $_COOKIE['dsrptv_user_ip_data'] ) && !empty( $_COOKIE['dsrptv_user_ip_data']) ){
			return json_decode( stripslashes( $_COOKIE['dsrptv_user_ip_data'] ), true );
		}
		...
		@setcookie( 'dsrptv_user_ip_data', json_encode( $mo_data ) );
